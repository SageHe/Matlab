...
D.Qtrue(1,1) = D.Qtrue(1,1)*0.1;
% D.Rtrue = D.Rtrue*100;
Xk_p = X_gt(1,:); %should this be X_gt?
X0 = [6678 0 0 r*sqrt(mu/r^3)]';
pert = [0 0.075 0 -0.021]';
X0 = X0+pert;
tspan = [0 10];
...
%     P_kp1_m = F(:,:,i)*Pk_p*F(:,:,i)' + Omega*D.Qtrue*Omega';
    ...
    y_kp1_m_loop = [];
    for a = 1:12
        y_kp1_m_loop = [y_kp1_m_loop;nom_meas_EKF(x_kp1_m,X_gt(:,i),tvec(i),a)];
    end
    y_kp1_m(:,i+1) = y_kp1_m_loop;
    stat_num = DST(y_kp1_m(:,i+1),D.ydata{i}(end,:)); 
...
%         y_kp1_m(:,i) = nom_meas_EKF(x_kp1_m,X_GT(end,:),tvec(i),s);
        e_y_kp1 = D.ydata{i}([1:3],j) - y_kp1_m([(3*s-2):3*s],i+1);
    ...
    tspan = tspan + 10;
    X0 = x_kp1_p(:,i);
    plot(tvec(i),X_GT(end,1),'o')
    hold on
    plot(tvec(i),X_gt(1,i),'x')
end
X_EKF = x_kp1_p;
figure
sgtitle('States of Spacecraft')
