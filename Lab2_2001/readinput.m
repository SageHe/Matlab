...
while line > 0
    ...
    switch inpblk
        ...
        case 1 % read number of joints, bars, reactions, and loads
            ...
            if numreact~=6 && numreact~=8; error('incorrect number of reaction forces');end %Originally 3
            % initialize arrays
            joints       = zeros(numjoints,3); %Originally 2
            connectivity = zeros(numbars,2);
            reacjoints   = zeros(numreact,1);
            reacvecs     = zeros(numreact,3); %Originally 2
            loadjoints   = zeros(numloads,1);
            loadvecs     = zeros(numloads,3); %Originally 2
            
            % check whether system satisfies static determiancy condition
%             if 3*numjoints - 6 ~= numbars %Originally 2*numjoints - 3
...
%             end
        ...
        case 3 % read connectivity of bars
            ...
            % check joint ids
            if max(tmp(2:3))>numjoints || min(tmp(2:3))<1 %Originally max(tmp(2:3)) and min(tmp(2:3))
