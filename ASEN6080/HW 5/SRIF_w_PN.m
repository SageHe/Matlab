...
Gamma = [dt^2/2 0 0;0 dt^2/2 0;0 0 dt^2/2;...
...
%         Ri_minus(:,:,i) = Ri_plus(:,:,i-1)*pinv(Phi(:,:,i));        %should be same as Ri_plus(:,:,i-1)*inv(Phi(:,:,i))
        Ri_car(:,:,i) = Ri_plus(:,:,i-1)*pinv(Phi(:,:,i));
        M = [Ru zeros(3,7);-Ri_car(:,:,i)*Gamma Ri_car(:,:,i) Deltab_plus(:,i-1)];
        [Q,R_transf] = qr(M);
        Ri_minus(:,:,i) = R_transf(4:9,4:9);
        Deltab_minus(:,i) = R_transf(4:9,end);
...
grid on
grid minor
sgtitle('SRIF w PN Position State Errors')
...
grid on
grid minor
sgtitle('SRIF w PN Velocity State Errors')
