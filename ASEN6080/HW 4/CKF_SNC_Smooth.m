...
%     state_error = x_hat - truth_state(:,1:6)';
    ...
    RMSvalsSNC = state_error.^2;
    RMSvalsSNC = sum(RMSvalsSNC,1);
    RMSvalsSNC = RMSvalsSNC./size(state_error,1);
    RMSvalsSNC = sqrt(RMSvalsSNC);
    % state_error(7,:) = [];
...
%     xlim([0 10e4])
    ...
    covbound(end+1,:) = [3*sqrt(P_i_l(1,1,end)) 3*sqrt(P_i_l(2,2,end)) 3*sqrt(P_i_l(3,3,end)) 3*sqrt(P_i_l(4,4,end)) 3*sqrt(P_i_l(5,5,end)) 3*sqrt(P_i_l(6,6,end))]; 
    
    clear state_error
    clear RMSvals
    state_error = (truth_state(:,1:6)' - x_hat_smoothed)';
    RMSvalsSmooth = state_error.^2;
    RMSvalsSmooth = sum(RMSvalsSmooth,1);
    RMSvalsSmooth = RMSvalsSmooth./size(state_error,1);
    RMSvalsSmooth = sqrt(RMSvalsSmooth);
