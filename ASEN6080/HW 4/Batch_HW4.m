...
R = 4*diag([1e-3 1e-6]);%diag([sigma_rho sigma_rhodot sigma_rho sigma_rhodot sigma_rho sigma_rhodot]);
...
state_error = truth_state(:,1:6) - x(:,1:6);
% state_error(7,:) = [];
...
grid on
grid minor
sgtitle('Position State Error with 3\sigma Covariance Bound, Batch')
...
grid minor
ylim([-.2e-3 .2e-3])
sgtitle('Velocity State Error with 3\sigma Covariance Bound, Batch')
