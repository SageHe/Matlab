...
% dx = zeros(7,1);
dx = [0.5 -0.5 0.5 0.5e-7 -0.5e-7 0.5e-7 0 0]';
...
for i = 2:numel(t)
    ...
    tspan = [0:dt/2:10];
    [~,X_k_minus_flat] = ode45(@(t,Z) keplerJ2_ODE(t,Z,n),tspan,Z,opts);
...
grid on
grid minor
sgtitle('UKF Position State Errors w/ PN')
...
grid on 
grid minor
sgtitle('UKF Velocity State Errors w/ PN')
