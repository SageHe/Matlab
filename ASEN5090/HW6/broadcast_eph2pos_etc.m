function [health,x,bsv,relsv] = broadcast_eph2pos_etc(ephem_all,t_input,prn)
...
%     jj = max( find(dt_input(tt) >= dt_ephem) ); % sat_ephem(:,17) = toe (sec into GPS week) of each entry
    ...
    delta_u = sat_ephem(jj,12)*sin(2*u) + sat_ephem(jj,11)*cos(2*u);
    delta_r = sat_ephem(jj,14)*sin(2*u) + sat_ephem(jj,13)*cos(2*u);
    delta_i = sat_ephem(jj,16)*sin(2*u) + sat_ephem(jj,15)*cos(2*u);
    
   % Compute corrected arugment of latitude
    u = u + delta_u;
    ...
    inc = sat_ephem(jj,7) + delta_i + sat_ephem(jj,10)*dt;     %  inclination 
    ...
    tau = -(2/c^2)*sqrt(a*muE)*ecc*sin(E);
    relsv(tt) = tau*c;
